...
export const createSupportTicket = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmail(
      ticket.email,
      `[IRAAP Support] Request #${ticket.id} received`,
      `<p>Hello ${ticket.fullName},</p><p>Your support request <strong>#${ticket.id}</strong> has been received. The IRAAP admin team will review it and get back to you.</p>`,
      "support",
    );
    ...
    await Promise.all(
      admins.map((admin) =>
        sendEmail(
          admin.email,
          `[IRAAP Support] New request #${ticket.id}`,
          `<p>A new support request was submitted by <strong>${ticket.fullName}</strong>.</p><p><strong>${ticket.subject}</strong></p><p style="white-space: pre-wrap;">${ticket.message}</p>`,
          "support",
        ),
      ),
    );
    ...
    return res
      .status(201)
      .json({ success: true, message: "Support request submitted", ticket });
  } catch (error) {
...
export const updateSupportTicket = async (req: Request, res: Response) => {
  ...
  if (!["OPEN", "IN_PROGRESS", "RESOLVED", "CLOSED"].includes(status))
    return errorResponse(res, 400, "Invalid support status");
  try {
    const [ticket] = await db
    ...
    if (ticket.requesterId) {
      ...
      await sendEmail(
        ticket.email,
        `[IRAAP Support] Request #${ticket.id} updated`,
        `<p>Hello ${ticket.fullName},</p><p>Your support request <strong>#${ticket.id}</strong> is now <strong>${ticket.status}</strong>.</p>${ticket.adminNote ? `<p>${ticket.adminNote}</p>` : ""}`,
        "support",
      );
    }
